...
/** Short name for the type of a value. */
...
/**
 * One line preview of a value. Nested values are only expanded one level, so a
 * preview stays short enough for a tree row.
 */
export function previewValue(value: unknown, depth = 0): string {
  ...
  if (Array.isArray(value)) {
    ...
    const items = value.slice(0, MAX_ENTRIES).map((item) => previewValue(item, depth + 1));
  ...
  const entries = keys.slice(0, MAX_ENTRIES).map((key) => {
    ...
    return `${key}: ${previewValue(inner, depth + 1)}`;
  });
